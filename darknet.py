...
class TestNet(torch.nn.Module):
    def __init__(self, num_layers, input_size):
        ...
        self.linear_1 = torch.nn.Linear(input_size, 5)
        self.middle = torch.nn.ModuleList([nn.Linear(5, 5) for _ in range(num_layers)])
        self.output = torch.nn.Linear(5, 2)

    def forward(self, x):
        x = x.view(-1)
        fwd = torch.nn.Sequential(self.linear_1, *self.middle, self.output)
        return fwd(x)


def get_test_input() -> torch.autograd.Variable:
    img = cv2.imread("dog-cycle-car.png")
    img = cv2.resize(img, (416, 416))
    img_ = img[:, :, ::-1].transpose((2, 0, 1))
    img_ = img_[np.newaxis, :, :, :] / 255.0
    img_ = torch.from_numpy(img_).float()
    return torch.autograd.Variable(img_)
...
class MaxPoolStride1(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        pooled_x = torch.nn.MaxPool2d(self.kernel_size, self.pad)(padded_x)
        return pooled_x
...
def create_modules(cfg_blocks: List[Dict]) -> Tuple[Dict, torch.nn.ModuleList]:
    net_info = cfg_blocks[0]  # Captures the information about the input and pre-processing
    ...
    for x in cfg_blocks:
        ...
        # shortcut corresponds to skip connection
            ...
            size = int(x["size"])
            if stride != 1:
                maxpool = torch.nn.MaxPool2d(size, stride)
            else:
...
class Darknet(torch.nn.Module):
    ...
    def save_weights(self, saved_file: str, cutoff=0):
